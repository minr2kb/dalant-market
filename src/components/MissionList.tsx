...
export function MissionList({ missions, marketId }: { missions: Mission[]; marketId: string }) {
    ...
    <div className="space-y-5">
      <div className="flex gap-2">
        {(['active', 'past'] as VisibleStatus[]).map((s) => (
          ...
          >
            {LABEL[s]}
            <span className={`text-[11px] tabular-nums ${status === s ? 'opacity-80' : 'text-gray-400'}`}>
              {counts[s]}
            </span>
          </button>
        ))}
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          <div className="py-12 text-center text-sm text-gray-400">{EMPTY[status]}</div>
        ) : (
