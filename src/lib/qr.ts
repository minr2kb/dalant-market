...
export type ParsedQR =
  | { type: 'mission'; marketId: string; missionId: string; userId: string; token: string; photoUrls?: string[] }
...
export function parseQR(value: string): ParsedQR {
  ...
  if (parts[1] === 'm' && parts.length === 7)
    return { type: 'mission', marketId: parts[2], missionId: parts[3], userId: parts[4], token: qrPart, photoUrls }
