...
const EMPTY_FORM = { title: '', reward: '', limitCount: '', type: 'qr' as MissionType }
  ...
  function submitForm() {
    ...
    if (editingId) {
      setMissions((prev) =>
        prev.map((m) =>
          m.id === editingId
            ? { ...m, title: form.title, reward: Number(form.reward), limitCount: Number(form.limitCount), type: form.type }
            : m,
        ),
      )
    } else {
      ...
      {showForm && (
        <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4 space-y-3">
          ...
          <div className="flex gap-2">
            {(['qr', 'upload', 'admin_grant'] as MissionType[]).map((t) => (
              <button
                key={t}
                type="button"
