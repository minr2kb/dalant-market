...
function AdminMissionsContent({ marketId }: { marketId: string }) {
  ...
  const createMutation = useMutation(missionsQuery.create({ invalidates: [missionsQuery.$key] }))
  const updateMutation = useMutation(missionsQuery.update({ invalidates: [missionsQuery.$key] }))
  const deleteMutation = useMutation(missionsQuery.delete({ invalidates: [missionsQuery.$key] }))
  ...
  async function toggleActive(missionId: string, current: boolean) {
    await updateMutation.mutateAsync({ marketId, missionId, isActive: !current })
  }
  ...
  async function submitForm() {
    ...
    if (editingId) {
      await updateMutation.mutateAsync({ marketId, missionId: editingId, ...body })
    } else {
            ...
            >
              <div className="flex items-center gap-3 px-4 py-4">
                ...
                <Switch
                  ...
                  onCheckedChange={() => toggleActive(mission.id, mission.isActive)}
